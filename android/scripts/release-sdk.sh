...
echo "Releasing Gedui Vox SDK ${SDK_VERSION}"
...
# Now build and publish the Gedui Vox SDK and its dependencies
echo "Building and publishing the Gedui Vox SDK"
pushd ${THIS_DIR}/../
./gradlew clean 
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Gedui Vox SDK + dependencies: ${SDK_VERSION}"
    popd
